...
class FIASObject with FIASObjectMappable {
  ...
  String get city => parents.firstWhereOrNull((element) => element.contentType == 'city')?.fullValue ?? '';
  String get district => parents.firstWhereOrNull((element) => element.contentType == 'district')?.fullValue ?? '';
  String get region => parents.firstWhereOrNull((element) => element.contentType == 'region')?.fullValue ?? '';
...
class FIASSubObject with FIASSubObjectMappable {
  FIASSubObject({required this.name, required this.typeShort, required this.type, required this.contentType});
  final String name;
  final String typeShort;
  final String type;
  final String contentType;
...
String getFullString(FIASObject v) {
  ...
  if (v.parents.any((element) => element.contentType == 'region')) {
    fs.add(v.parents.firstWhere((element) => element.contentType == 'region').fullValue);
  }
  if (v.parents.any((element) => element.contentType == 'district')) {
    fs.add(v.parents.firstWhere((element) => element.contentType == 'city').fullValue);
  }
